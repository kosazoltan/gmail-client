...
export function deleteProtection(req: Request, res: Response, next: NextFunction): void {
  ...
  for (const pattern of LOCAL_DB_DELETE_ROUTES) {
    if (pattern.test(path)) {
      logger.warn(
        `[DELETE-PROTECTION] LOCAL DB DELETE: ${req.method} ${path} — body: ${JSON.stringify(req.body).substring(0, 200)}`,
      );
      next();
      return;
    }
  ...
  logger.warn(
    `[DELETE-PROTECTION] UNKNOWN DELETE route: ${req.method} ${path} — allowing but monitoring`,
  );
  next();
}
