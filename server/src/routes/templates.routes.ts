...
router.get('/', async (req, res) => {
  try {
    ...
    const templates = await queryAll<Template>(
    ...
    logger.error('Sablonok lekérése hiba:', error);
...
router.post('/', async (req, res) => {
  try {
    ...
    logger.error('Sablon létrehozása hiba:', error);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existing = await queryOne<Template>('SELECT * FROM templates WHERE id = ? AND account_id = ?', [
      id,
      accountId,
    ]);
    ...
    logger.error('Sablon frissítése hiba:', error);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await execute('DELETE FROM templates WHERE id = ? AND account_id = ?', [id, accountId]);

    res.json({ success: true });
  } catch (error) {
    logger.error('Sablon törlése hiba:', error);
