...
test.describe('Scan — Sync Button & Progress', () => {
  ...
  test('sync/refresh button is visible and clickable in header', async ({ page }) => {
    ...
    const syncButton = page.locator('button').filter({ has: page.locator('svg.lucide-refresh-cw') });
  ...
  test('sync button shows loading state while syncing', async ({ page }) => {
    ...
    const spinningIcon = page.locator('svg.lucide-refresh-cw.animate-spin');
    const isLoading = await spinningIcon
      .or(page.locator('svg.lucide-loader-2.animate-spin'))
      .isVisible()
      .catch(() => false);
