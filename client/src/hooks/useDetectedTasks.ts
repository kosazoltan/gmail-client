...
import { refreshDashboardAndDetectedTasks } from '../lib/refreshAfterEmailChange';

export function useDetectedTasks(params?: {
  status?: string;
  priority?: string;
  page?: number;
  limit?: number;
...
export function useScanTasks() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await refreshDashboardAndDetectedTasks(queryClient);
    },
  });
}
...
export function useSnoozeDetectedTask() {
  ...
  return useMutation({
    mutationFn: ({ id, days }: { id: string; days: number }) => api.detectedTasks.snooze(id, days),
    onSuccess: async () => {
      await refreshDashboardAndDetectedTasks(queryClient);
    },
  });
