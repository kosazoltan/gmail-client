...
const MEMORY_ROOT = dirname(fileURLToPath(import.meta.url));
...
function buildSearchableText(chunk) {
  ...
  if (chunk.retrieval_hints) {
    if (chunk.retrieval_hints.lexical) {
      parts.push(
        ...(Array.isArray(chunk.retrieval_hints.lexical) ? chunk.retrieval_hints.lexical : []),
      );
    }
    if (chunk.retrieval_hints.semantic) {
      parts.push(
        ...(Array.isArray(chunk.retrieval_hints.semantic) ? chunk.retrieval_hints.semantic : []),
      );
    }
...
const queryParts = args.filter(
  (arg, index) => !consumedArgIndexes.has(index) && !arg.startsWith('--'),
);
